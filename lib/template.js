...
var template = {
    HTML: function (title, list, body, control, authStatusUI='<a href="/auth/login">로그인</a>',comment,commentcontrol) {
      ...
      <body>
        ...
        <script src="/comment_control.js"></script>
      </body>
      </html>
      `;
  ...
  for (var i = 0; i < comment.length; i++) {
      ...
      var time = hours + ':' + minutes;
      var form
      if(comment[i].author_id == 1){
        form = `                
        <form action="/comment/delete_anony_comment" method="post">
          <input type="hidden" class="author_id" name="author_id" value="${comment[i].author_id}">
          <input type="hidden" class="comment_pwd" name="password" value="${comment[i].password}">
          <input type="hidden" class = "post_id" name="post_id" value="${comment[i].post_id}">
          <input type="hidden" class="comment_id" name="comment_id" value="${comment[i].comment_id}">
          <input type="submit" class="comment-delete-btn" value="삭제">
        </form>`
      }
      else{
        form = `                
